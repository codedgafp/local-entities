    ...
    public function display($course, $section) {
        ...
        if ($datas = $form->get_data()) {
            ...
            try {
                ...
                if(isset($datas->defaultentitychanged) && $datas->defaultentitychanged) {
                    $defaultentityinfo = '<div class="alert alert-info">'
                                . get_string('default_entity_changed', 'local_entities')
                                . '</div>';
                }
        ...
        if ($entity->is_main_entity()) {
            ...
            if ($presentationpageurl) {
                $presentationid = null !== $presentationpageurl->get_param('id') ? $presentationpageurl->get_param('id') : '';
                $presentation = $DB->get_record('course', array('id' => $presentationid));
                $buttonText = $presentation->visible == 1 ? get_string('hidepresentationpage', 'local_entities') : get_string('showpresentationpage', 'local_entities');
                ...
                echo '<div id="hidepresentationpage" data-visible="' . $presentation->visible . '" data-courseid="' . $presentationid . '"><a "javascript:void(0)" class="btn-secondary">' .
                     $buttonText
                     . '</a></div>';
        ...
        $this->page->requires->strings_for_js([
            ...
            'hidepresentationpage',
            'default_entity_changed',
            'presentationpagevisibilityupdated',
